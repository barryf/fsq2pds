...
- **`vt push` pushes everything including `.git/`**: Val.Town supports a `.vtignore` (works like `.gitignore`) to exclude files. Created `.vtignore` to limit pushes to runtime files only: `cron.ts`, `lib/`, `deno.json`, `README.md`. Excluded: `.git/`, `.gitignore`, `.claude/`, `PLAN.md`, `CLAUDE.md`, `bootstrap/`, `lexicons/`.
